...
const guys = people.sort((guy1, guy2) => guy1.score - guy2.score)
// console.log(guys)
...
const filter = animals.filter(item => item.length == 3)
// console.log(filter)
...
const length = animals.map(animal => animal.length)
// console.log(length)
...
const numeros = numbers.filter(number => number%2 != 0).map(item => item)
const numerosSoma = numeros.reduce((acc,item) => {
    return acc + item
}, 0)
...
const filter2 = data.filter(pais => pais.country !== 'China')
const sum = filter2.reduce((acc,item) => {
